...
struct PowerDifference {
	__host__ __device__ float operator()(const float& a, const float& b) const { return a * b; }
};
...
int main()
{
	...
	const int N		= 128;			// --- Number of vector elements
	const int Nvec	= 5;			// --- Number of vectors for each matrix
    const int Nvec2 = 5 ; 
	...
	thrust::device_vector<float> d_matrix2(Nvec2* N);
