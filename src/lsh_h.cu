...
//    std::uniform_int_distribution<> dis( 0, 127);
    ...
    {
             temp[i] =static_cast< float >(distrib(gen)); 
...
// make random data 
void make_vector_h_sum_1(des_t_h &vec)
{
   int dim = 128 ; 
    float temp[128] ;  
    float t = 0 ;    
    ...
    //
    // std::uniform_int_distribution<> zero(0, 127);
    // for (int i = 0; i < 60; i++)
    // {
    //     temp[zero(gen)] = 0; 
    // }
...
void make_vec_h(int nbits, des_t_h2 * vec, int type)
{
...
__device__ inline void atomic_min2_update(min2_index * min_2_index, half2 min2, uint32_t idx, int index, uint32_t * matches)
{
   ...
   do {
        ...
        // to avoid having the same numbers in the min2 we compare for index 2 
        else if(__hgt(min2_test.min_index.min2.y, min2.x) && (min2_test.min_index.index != idx))
        {
...
// class used to sort with thrust  
class IndexCompare_int
{
    thrust::device_ptr<int> _code ;

public:
    IndexCompare_int(thrust::device_ptr<int> code)
        :  _code( code)
    { }
    __device__
...
{   
    ...
    {
        // printf("%i one , %i two %i three %i four ", one, two, three, four) ;
    }
}

//sets n bits for each value 
// __global__ void set_bit_h_n(int * buckets, int nbits, half2 * dot, int size)
// {
//     uint32_t var = 0 ; 
...
//                 temp1 = 0 ; 
//             }
//             else if(__hgt(410, temp.x))
//             {

//                 atomicAdd(&two, 1) ; 
//                 temp1 = 1 ; 
//             }
//             else if(__hgt(455, temp.x))
//             {
//                 atomicAdd(&three, 1) ; 
//                 temp1 = 2 ; 
//             }
...
//                 temp2 = 0 ; 
//             }
//             else if(__hgt(410, temp.y))
//             {
//                 temp2 = 1 ; 
//             }
//             else if(__hgt(455, temp.y))
//             {
//                 temp2 = 2 ; 
//             }
//             else
...
//reads in the dot prduct values and usese them to make the buckets
//each thread will make its own value, we get both coalsed reads and writes 
...
//        printf("%i var \n", var) ; 
        buckets[warp_idx + threadIdx.x] = var ; 
        // also set the index array which we will need when we sort 
        index[warp_idx + threadIdx.x] =  warp_idx + threadIdx.x ; 
    }

}

//each thread will make its own value, this means we get both coalsed reads and writes 
__global__ void set_bit_h_range(int * buckets, int nbits, half2 * dot, int size, int  * check_val)
{
...
// takes 2 buckets and finds the 2nns for each point from the query bucket 
// called with 
// grid, y, 1, 1 
// block 32, x, 1 
...
{   
   ...
   // } 
    ...
    set_bit_h<<<grid_bit, block_bit, shared_size, stream>>>(code_dev, rand_array_size, dot_res, size, index_dev) ; 
    

    cudaStreamSynchronize ( stream ) ; 
    IndexCompare_int code_sort(thrust::device_pointer_cast(code_dev));
    ...
    for(int i = 1; i < size; i++)
    {
        ...
        {
        //    printf("%i %i start = %i index i = %i \n ", index_host[i], code_host[index_host[i]] ,start,i ) ; 
            buckets[count].y += 1  ; 
        }
        else
        {
                    ...
                    int2 * buckets_q, int4 * brute, min2_index * min_2_index, cudaStream_t * stream, int stream_index )
{
    ...
    std::thread q_thread(lsh_thread_sort_reduce, Q, q_n, rand_array, nbits,
                dot_res_q, code_q_host, code_q_dev, index_q_host, index_q_dev,  buckets_q, stream[stream_index * 2], std::ref(buckets_size_q));   

    std::thread r_thread(lsh_thread_sort_reduce, R, r_n, rand_array, nbits,
...
// main function 
//number of streams should be % 2 = 0 and less than 16 
int lsh_gpu(void * q_points, void * r_points, int type, uint32_t q_n, uint32_t r_n, uint32_t * matches, float threshold, cublasHandle_t handle, int stream_n, int l, int lsh_type, int nbits) 
{
    ...
    // if we only run one iteration more streams are ussless 
    if(l == 1)
    {
        stream_n = 2 ;
    }
    ...
    // will just use zero copy assumese that memory is mapped and pinned 
    if (type == FLOAT_HOST)
    {
        float * r_copy ; 
        float * q_copy ; 
...
//    thrust::constant_iterator<int> array_of_ones(1) ; 
 ...
 //       cudaMemsetAsync(rand_array[i], 0, sizeof(des_t_h2) * rand_array_size, stream[i * 2]) ;  
        ...
        memset(rand_array_host[i], 0, sizeof(des_t_h2) * rand_array_size) ; 
...
//        cudaMallocAsync((void **)&code_r[i], sizeof(int) *r_n , stream[ i * 2 + 1] ) ; 
    ...
    while((in_use_threads < number_threads) && (in_use_threads < l))
    {
            ...
            make_vec_h(nbits, rand_array_host[in_use_threads], 0);
            ...
            //  cublasStatus_t stat =
            cublasHgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, rand_array_size, q_n, 128, &a, (half *)rand_array_dev[in_use_threads], 128, (half *)Q, 128, &b, (half *)dot_res_q[in_use_threads], rand_array_size);

            cudaStreamSynchronize(stream[in_use_threads * 2 + 1])  ; 
            cublasSetStream(handle, stream[in_use_threads * 2 + 1]) ; 
            // stat =
            cublasHgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, rand_array_size, r_n, 128, &a, (half *)rand_array_dev[in_use_threads], 128, (half *)R, 128, &b, (half *)dot_res_r[in_use_threads], rand_array_size);
    ...
    // wait for thread the start new thread untill all iterations are done 
    ...
    cudaFree(min_2_index); 
    cudaFree(R) ; 
    cudaFree(Q) ; 

    for (int i = 0; i < stream_n/2; i++)
    {
        cudaFreeHost(rand_array_host[i]) ; 
        cudaFree(rand_array_dev[i]) ; 

        cudaFree(dot_res_q[i]) ; 
        cudaFree(dot_res_r[i]) ; 

        cudaFree(index_q_dev[i]) ; 
        cudaFree(index_r_dev[i]) ; 

        cudaFreeHost(index_q_host[i]) ; 
        cudaFreeHost(index_r_host[i]) ; 

        cudaFree(code_q_dev[i]) ; 
        cudaFree(code_r_dev[i]) ; 

        cudaFreeHost(code_q_host[i]) ; 
        cudaFreeHost(code_r_host[i]) ; 

        cudaFreeHost(buckets_q[i]) ; 
        cudaFreeHost(buckets_r[i]) ; 
        
        cudaFreeHost(brute_host[i]) ; 
